...
def build_alerts(df: pd.DataFrame) -> list[str]:
    ...
    if "Prioritas" in df.columns:
        tinggi_count = int((df["Prioritas"] == "PRIORITAS TINGGI").sum())
        if tinggi_count >= 3:
    ...
    if "Topik" in df.columns:
        topik_counts = df["Topik"].astype(str).value_counts()
    ...
    if "Provinsi" in df.columns:
        prov_counts = df["Provinsi"].astype(str).str.strip()
    ...
    if "Waktu_Publish_WIB" in df.columns:
        try:
            publish_dt = normalize_datetime_col(df, "Waktu_Publish_WIB")
            now = pd.Timestamp.now()
            recent = df[publish_dt >= (now - pd.Timedelta(hours=6))]
            if len(recent) >= 3:
...
def guide_card(title: str, content: str):
    st.markdown(
        f"""
        <div class="info-card">
            <div style="font-size:1rem; font-weight:800; margin-bottom:8px; color:#0f172a;">{escape(title)}</div>
            <div style="font-size:14px; line-height:1.75; color:#334155;">{content}</div>
        </div>
        """,
        unsafe_allow_html=True
    )
...
def ensure_publish_date(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.copy()
    df["Tanggal_Hari"] = pd.to_datetime(s, errors="coerce").dt.date
    return df.dropna(subset=["Tanggal_Hari"]).copy()
...
with c_ctrl1:
    ...
    if st.button("🔄 Update Data", key="update_data_main", use_container_width=True):
        ...
        st.rerun()
with c_ctrl2:
    date_range = st.date_input("Rentang tanggal", value=(min_date, max_date), key="main_date_range")
with c_ctrl3:
    filter_option = st.selectbox("Prioritas", ["SEMUA", "PRIORITAS TINGGI", "PRIORITAS SEDANG", "PRIORITAS RENDAH"], key="main_filter_option")
with c_ctrl4:
    kategori_option = st.selectbox("Kategori Berita", ["SEMUA", "NASIONAL", "GLOBAL", "EDUKASI"], key="main_kategori_option")

st.markdown(
    f"<div style='margin-top:10px; font-size:12px; color:#64748b;'>Periode data tersedia: <b>{escape(str(min_date))}</b> s.d. <b>{escape(str(max_date))}</b></div>",
    unsafe_allow_html=True
)
...
# ===============================
TOPIC_RULES = {
    "PHK": [r"\bphk\b", r"pemutusan hubungan kerja", r"\bdirumahkan\b", r"gelombang phk", r"phk massal", r"pengurangan karyawan", r"efisiensi tenaga kerja"],
    ...
    "Upah / Gaji": [r"\bupah\b", r"\bgaji\b", r"tunggakan upah", r"gaji tidak dibayar", r"ump", r"umk", r"upah minimum"],
    "Aksi / Demo Buruh": [r"\bdemo\b", r"unjuk rasa", r"aksi buruh", r"mogok", r"mogok kerja"],
    "Konflik Hubungan Industrial": [r"perselisihan", r"konflik buruh", r"sengketa", r"tripartit", r"mediasi hubungan industrial"],
    "Kepesertaan BPJS": [r"bpjs ketenagakerjaan", r"bpjamsostek", r"jamsostek", r"kepesertaan bpjs", r"terdaftar bpjs", r"peserta bpjs"],
    ...
    "Jaminan Pensiun (JP)": [r"\bjp\b", r"jaminan pensiun", r"manfaat pensiun", r"iuran pensiun", r"usia pensiun"],
    "Kecelakaan Kerja (JKK)": [r"\bjkk\b", r"jaminan kecelakaan kerja", r"kecelakaan kerja", r"santunan jkk", r"ledakan pabrik", r"buruh tewas", r"pekerja tewas"],
    ...
    "Pekerja Migran Indonesia (PMI)": [r"\bpmi\b", r"pekerja migran", r"tki", r"buruh migran"],
...
raw_filtered = raw[(raw["Tanggal_Hari"] >= start_date) & (raw["Tanggal_Hari"] <= end_date)].copy()
filtered_display = analyzed[(analyzed["Tanggal_Hari"] >= start_date) & (analyzed["Tanggal_Hari"] <= end_date)].copy() if not analyzed.empty else pd.DataFrame()

if not filtered_display.empty and kategori_option != "SEMUA" and "Kategori_Berita" in filtered_display.columns:
    filtered_display = filtered_display[filtered_display["Kategori_Berita"].astype(str).str.upper().eq(kategori_option)].copy()

if not filtered_display.empty:
    if "Topik_Utama" in filtered_display.columns:
        filtered_display["Topik"] = filtered_display["Topik_Utama"].astype(str).fillna("").replace("", "Lainnya")
    else:
...
if not filtered_for_table.empty and filter_option != "SEMUA" and "Prioritas" in filtered_for_table.columns:
    filtered_for_table = filtered_for_table[filtered_for_table["Prioritas"].astype(str).eq(filter_option)].copy()

# ===============================
# TABS
# ===============================
tab_dash, tab_data, tab_region, tab_info = st.tabs(["📊 Dashboard Utama", "📰 Data Berita", "📍 Analisis Wilayah", "📘 Panduan"])
...
with tab_dash:
    ...
    if filtered_display.empty or "Prioritas" not in filtered_display.columns:
        section_header("Ringkasan Utama", "Gambaran cepat kondisi isu pada periode yang dipilih.")
    ...
    section_header("Isu Paling Kritis Hari Ini", "Berita dengan urgensi tertinggi berdasarkan skor, waktu publikasi, dan konteks analisis.")
    ...
    if not df_critical.empty:
        ...
        lokasi = escape(clean_label(str(top_issue.get("Kabupaten_Kota", "") or "").strip() or str(top_issue.get("Provinsi", "") or "").strip() or "-"))
        ...
        title_html = judul if not link else f"<a href='{escape(link, quote=True)}' target='_blank'>{judul}</a>"
        card_html = (
            ...
            f"<span class='pill pill-blue'>{kategori}</span>"
            f"<span class='pill pill-soft'>{lokasi}</span>"
            f"<span class='pill pill-soft'>Topik: {topik}</span>"
            f"<span class='pill pill-soft'>Dampak: {dampak}</span>"
            "</div>"
            f"<div class='news-body'>{alasan}</div>"
            "</div>"
        )
    ...
    with left:
        ...
        if not priority_counts.empty:
            ...
            label_map = {"PRIORITAS TINGGI": "Prioritas Tinggi", "PRIORITAS SEDANG": "Prioritas Sedang", "PRIORITAS RENDAH": "Prioritas Rendah"}
            color_map = {"PRIORITAS TINGGI": "#ef4444", "PRIORITAS SEDANG": "#f59e0b", "PRIORITAS RENDAH": "#22c55e"}
            ...
            fig = go.Figure()
            fig.add_trace(go.Bar(x=x_vals, y=y_vals, orientation="h", marker=dict(color=colors, line=dict(width=0)), text=[f"{v:,}" for v in x_vals], textposition="outside", cliponaxis=False, hovertemplate="<b>%{y}</b><br>Jumlah berita: %{x}<extra></extra>"))
            fig.update_layout(height=300, margin=dict(l=10, r=45, t=6, b=20), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", showlegend=False, xaxis=dict(title="Jumlah Berita", showgrid=True, gridcolor="rgba(148, 163, 184, 0.20)", zeroline=False, showline=False, range=[0, max_val * 1.18]), yaxis=dict(title="", autorange="reversed", showgrid=False), font=dict(size=13))
        ...
        if not topik_counts.empty:
            fig_topik = go.Figure()
            fig_topik.add_trace(go.Bar(x=topik_counts.values.tolist(), y=[clean_label(x) for x in topik_counts.index.tolist()], orientation="h", text=[f"{v:,}" for v in topik_counts.values.tolist()], textposition="outside", marker=dict(color="#1d4ed8")))
            fig_topik.update_layout(height=320, margin=dict(l=10, r=45, t=6, b=20), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", showlegend=False, yaxis=dict(autorange="reversed", title=""), xaxis=dict(title="Jumlah Berita", showgrid=True, gridcolor="rgba(148,163,184,0.20)"))
        ...
        if not df_high.empty:
            ...
            for _, row in df_high.head(5).iterrows():
                media = escape(str(row.get("Media", "-")))
                judul = escape(str(row.get("Judul", "-")))
                link = str(row.get("Link", "")).strip()
                waktu = escape(str(row.get("Waktu_Publish_WIB", "")))
                lokasi = escape(clean_label(str(row.get("Kabupaten_Kota", "") or "").strip() or str(row.get("Provinsi", "") or "").strip() or "-"))
                kategori = escape(clean_label(row.get("Kategori_Berita", "-")))
                title_html = judul if not link else f"<a href='{escape(link, quote=True)}' target='_blank'>{judul}</a>"
                st.markdown(f"<div class='news-card'><div class='news-title'>{title_html}</div><div class='news-meta'>{media} • {waktu} • {lokasi} • {kategori}</div></div>", unsafe_allow_html=True)
        else:
    ...
    with right:
        ...
        if "Topik" in filtered_display.columns and not filtered_display.empty:
            topik_counts = filtered_display["Topik"].value_counts().head(3)
            topik_text = "\n".join([f"- **{clean_label(topic)}** ({count} berita)" for topic, count in topik_counts.items()])
            topik_utama = topik_counts.index.tolist()
        else:
            topik_text = "- **Belum ada topik dominan**"
            topik_utama = []
        ringkasan_utama, dampak_utama = [], []
...
""",
        ...
        for msg in build_alerts(filtered_display):
            st.markdown(f"<div class='news-card'><div class='news-title'>{escape(msg)}</div></div>", unsafe_allow_html=True)
...
with tab_data:
    ...
    if search_keyword:
        ...
        st.markdown(f"<div style='font-size:12px; color:#475569; margin-top:6px; margin-bottom:10px;'>Menampilkan hasil untuk kata kunci: <b>{escape(search_keyword)}</b></div>", unsafe_allow_html=True)
    ...
    if "Waktu_Publish_WIB" in df_display.columns:
        ...
        sort_col = "Urutan"
    df_display = df_display.sort_values(["Urutan", sort_col], ascending=[True, False]).drop(columns=["Urutan"]).reset_index(drop=True)
    ...
    section_header("Berita Terkini", "Ringkasan berita terpilih untuk penelusuran cepat oleh pengguna dan pimpinan.")
    for i, row in df_page.iterrows():
        ...
        title_html = judul if not link else f"<a href='{escape(link, quote=True)}' target='_blank'>{judul}</a>"
        link_html = "" if not link else f"<div class='news-link'><a href='{escape(link, quote=True)}' target='_blank'>Buka berita sumber</a></div>"
        card_html = (
            "<div class='news-card'>"
            "<div style='display:flex; justify-content:space-between; gap:14px; flex-wrap:wrap; align-items:flex-start;'>"
            "<div style='flex:1; min-width:260px;'>"
            f"<div class='news-title'>{i + 1}. {title_html}</div>"
            f"<div class='news-meta'>{media} • {waktu}</div>"
            "</div>"
            f"<div>{badge_html(prioritas)}</div>"
            "</div>"
            ...
            f"{''.join(chips)}"
            "</div>"
            f"<div class='news-body' style='margin-bottom:10px;'>{alasan if alasan else 'Belum ada analisis prioritas.'}</div>"
            f"{link_html}"
            "</div>"
        )
        st.markdown(card_html, unsafe_allow_html=True)

    st.markdown(f"<div style='text-align:center; margin-top:10px; color:#64748b; font-size:12px;'>Menampilkan <b>{min(start_idx + 1, total_rows)}</b>–<b>{min(end_idx, total_rows)}</b> dari <b>{total_rows}</b> berita</div>", unsafe_allow_html=True)
    ...
    with nav3:
        if st.button("Berikutnya ➡", disabled=(st.session_state.page >= total_pages), use_container_width=True):
            ...
            st.rerun()
    with nav2:
        st.markdown(f"<div style='text-align:center; color:#64748b; font-size:12px; padding-top:10px;'>Halaman <b>{st.session_state.page}</b> dari <b>{total_pages}</b></div>", unsafe_allow_html=True)

    st.markdown("<div style='height:12px;'></div>", unsafe_allow_html=True)
    section_header("Indeks Eskalasi Isu", "Perbandingan intensitas pemberitaan 24 jam terakhir terhadap periode 24–48 jam sebelumnya.")
    ...
    def agg(df_recent):
        ...
        out = df_recent.groupby("Topik", dropna=False).agg(**{"Berita 24 Jam": ("Judul", "count"), "Media 24 Jam": ("Media", pd.Series.nunique)}).reset_index()
        head = (
...
with tab_region:
    ...
    if filtered_display.empty:
        ...
        if "Provinsi" in df_region_all.columns:
            provinsi_options += sorted([x for x in df_region_all["Provinsi"].astype(str).fillna("").unique() if x.strip()])
        ...
        if "Kabupaten_Kota" in df_region.columns:
            kab_options += sorted([x for x in df_region["Kabupaten_Kota"].astype(str).fillna("").unique() if x.strip()])
        with r2:
        ...
        section_header("Ringkasan Wilayah", "Gambaran cepat jumlah isu dan komposisi prioritas pada wilayah terpilih.")
        ...
        with lcol:
            ...
            if not df_region.empty:
                ...
                if not topik_region.empty:
                    fig_reg = go.Figure()
                    fig_reg.add_trace(go.Bar(x=topik_region.values.tolist(), y=[clean_label(x) for x in topik_region.index.tolist()], orientation="h", text=[f"{v:,}" for v in topik_region.values.tolist()], textposition="outside", marker=dict(color="#1d4ed8")))
                    fig_reg.update_layout(height=320, margin=dict(l=10, r=45, t=6, b=20), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", showlegend=False, yaxis=dict(autorange="reversed", title=""), xaxis=dict(title="Jumlah Berita", showgrid=True, gridcolor="rgba(148,163,184,0.20)"))
                    st.plotly_chart(fig_reg, use_container_width=True, config={"displayModeBar": False}, key="chart_topik_wilayah")
                else:
        ...
        with rcol:
            ...
            if not df_region.empty:
                ...
                fig_pr = go.Figure()
                fig_pr.add_trace(go.Bar(x=["Tinggi", "Sedang", "Rendah"], y=prio_region.values.tolist(), marker=dict(color=["#ef4444", "#f59e0b", "#22c55e"]), text=[f"{v:,}" for v in prio_region.values.tolist()], textposition="outside"))
                fig_pr.update_layout(height=320, margin=dict(l=10, r=20, t=6, b=20), paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", showlegend=False, xaxis=dict(title=""), yaxis=dict(title="Jumlah Berita", showgrid=True, gridcolor="rgba(148,163,184,0.20)"))
        ...
        if total_region == 0:
            ...
            region_summary_html = (
                "<div class='panel-card'>"
                "<div class='news-body'>"
                f"<p><b>Wilayah terpilih:</b> {escape(str(selected_label))}</p>"
                f"<p><b>Status:</b> {status_region}</p>"
                f"<p><b>Topik dominan:</b> {escape(topik_narasi)}</p>"
                f"<p><b>Rekomendasi:</b> {escape(rekom_region)}</p>"
                "</div>"
                "</div>"
            )
            st.markdown(region_summary_html, unsafe_allow_html=True)

        st.markdown("<div style='height:12px;'></div>", unsafe_allow_html=True)
        section_header("Berita Wilayah Terbaru", "Daftar berita teratas pada wilayah yang dipilih.")
        if not df_region.empty:
            ...
            for _, row in df_region_show.head(20).iterrows():
                ...
                card_html = (
                    ...
                    f"<div class='news-title'>{title_html}</div>"
                    f"<div class='news-meta'>{media} • {waktu} • {lokasi}</div>"
                    "</div>"
                    f"<div>{badge_html(prioritas)}</div>"
                    "</div>"
                    "<div style='margin:8px 0 10px 0;'>"
                    f"<span class='pill pill-soft'>Topik: {topik}</span>"
                    "</div>"
                    f"<div class='news-body'>{alasan if alasan else 'Belum ada analisis prioritas.'}</div>"
                    "</div>"
                )
                st.markdown(card_html, unsafe_allow_html=True)
        else:
...
# ===============================
with tab_info:
    section_header("Panduan Penggunaan", "Penjelasan ringkas mengenai fungsi dashboard, data berita, analisis wilayah, dan cara membaca prioritas isu.")
    guide_card("1. Gambaran Umum Dashboard", """
        ...
        """)
    guide_card("2. Sumber dan Alur Data", """
    ...
    guide_card("4. Ringkasan Utama", """
        ...
        """)
    guide_card("5. Isu Paling Kritis Hari Ini", """
        Bagian ini menampilkan satu isu yang dinilai paling kritis pada periode berjalan. Penentuan dilakukan dengan melihat
        skor analisis, waktu publikasi, dan konteks pemberitaan.
        """)
    guide_card("6. Distribusi Prioritas dan Topik Dominan", """
    ...
    guide_card("7. Data Berita", """
        ...
        dan alasan prioritas.
        """)
    guide_card("8. Indeks Eskalasi Isu", """
    ...
    guide_card("9. Analisis Daerah", """
        ...
        """)
    guide_card("10. Cara Membaca Prioritas", """
        ...
        tetap relevan, namun belum memerlukan perhatian segera.
        """)
    guide_card("11. Catatan Penggunaan", """
